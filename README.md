...
Для пакетного сбора передайте несколько фраз или файл с одной фразой на строку:

```bash
wordstat collect "ремонт квартир" "дизайн кухни" --phrases-file phrases.txt
```

Все фразы обрабатываются в одной браузерной сессии. Для каждой создаётся свой
каталог; ошибка одной фразы не останавливает остальные, но команда завершается
с ненулевым кодом и печатает сводку.
