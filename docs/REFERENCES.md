...
## ML и «AI-функции» в нише

Проверено по деревьям файлов и файлам зависимостей всех перечисленных выше
проектов (`requirements.txt`, `pyproject.toml`, `package.json`), а не по
описаниям.

**Классического машинного обучения нет ни у одного инструмента сбора.** Ни
`scikit-learn`, ни `numpy`/`scipy`, ни `transformers`/`sentence-transformers`,
ни `pymorphy`/`natasha`/`spacy`. Ни эмбеддингов, ни k-means/DBSCAN, ни
лемматизации. У всех парсеров, API-клиентов и MCP-серверов ниши ML-зависимостей
нет вовсе.

«Интеллект» есть ровно у двух проектов, и оба — не сборщики, а платформы поверх
уже собранных данных:

- [AndreyTsibin/keywai](https://github.com/AndreyTsibin/keywai) (5★, Django +
  React, ~2100 файлов) — кластеризация в
  `backend/apps/clustering/services/ai_clustering.py` (1108 строк): системный
  промпт «Ты SEO-эксперт с 15+ лет опыта…», список ключей уходит в user-промпт,
  вызов Claude через внутренний `AIGateway`, разбор JSON-ответа. Рядом
  `ai_filtering.py` и отдельная «пре-кластеризация» для больших объёмов. Это
  промпт-инжиниринг с кэшированием системных промптов, а не обучение модели.
- [Horosheff/yadryshko-semantic-core-subagent](https://github.com/Horosheff/yadryshko-semantic-core-subagent)
  (14★) — 18 файлов, питоновского ядра нет: два скрипта собирают XLSX/HTML-отчёты,
  всё остальное — markdown-инструкции для агента Cursor. Заявленная в описании
  кластеризация — это то, что делает LLM, читая методичку.

Практический вывод: типовые «ML-функции» SEO-тулинга (кластеризация семантики,
разметка интентов, прогноз сезонности) в этой нише либо отсутствуют, либо
сводятся к «отправь список ключей в LLM». **Добавлять ML в `wordstat-cli` не
нужно.** Это сборщик данных с гарантией «что скачано, то и записано, а типы
видны в манифесте»; кластеризация — отдельный слой поверх Parquet, который
одинаково хорошо делается pandas, внешним инструментом или тем же LLM.
Зависимость вроде `sentence-transformers` утяжелила бы установку и размыла
границу ответственности.

Обратная сторона того же наблюдения: обе платформы выше вынуждены начинать с
чистки сырых CSV/JSON. Типизированный Parquet на выходе — это и есть вклад
`wordstat-cli` в такой пайплайн, и он полезнее встроенной кластеризации.
...
- **ML добавлять не нужно** — его нет ни у одного сборщика в нише, а «AI» у
  двух платформ поверх данных сводится к вызову LLM с промптом. Кластеризация
  живёт слоем выше Parquet.
