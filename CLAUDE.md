...
- **`collector.py` (`WordstatCollector.collect_many`)** — единственное место,
  ...
  батча), ввести фразу → выбрать регион (переустанавливается на каждой фразе —
  идемпотентен) → для каждого из четырёх видов отчёта (`WordstatView`)
  переключить вкладку, скачать CSV, распарсить, записать `<view>.parquet` и
  убрать исходную загрузку.
