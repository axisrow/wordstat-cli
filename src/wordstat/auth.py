...
def read_profile_cookies(cookies_db: Path, key: bytes) -> tuple[list[dict[str, Any]], ImportReport]:
    ...
    The database is opened read-only and immutable, so a running Chrome keeps
    working and no locks are taken; transient OperationalErrors from a
    concurrent write are retried.
    """
    ...
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                raise SessionImportError(f"Could not open {cookies_db} (is the path accessible?)") from None
            time.sleep(0.2)
    ...
    try:
        try:
            rows = connection.execute(
                ...
                " has_expires, is_secure, is_httponly, samesite FROM cookies"
            ).fetchall()
...
async def import_session(cdp_url: str, profile_dir: Path) -> ImportReport:
    ...
    async with _attached_session(cdp_url) as (session, page):
        ...
        stored_keys = {
            (cookie["name"], cookie["domain"].lstrip("."), cookie["path"]) for cookie in stored
        }
...
async def logout_session(cdp_url: str) -> LogoutReport:
    ...
    The mirror of import_session. `Storage.deleteCookies` does not exist on
    this Chrome's browser endpoint, so deletion is done the proven way:
    overwrite each Yandex cookie via root `Storage.setCookies` with an already
    expired timestamp (Chrome evicts it immediately), keyed by the exact
    stored (name, domain, path) — unrelated cookies of the profile survive.
    Afterwards the attached Chrome is a logged-out profile again, restorable
    with `wordstat login`.
    """
    ...
    async with _attached_session(cdp_url) as (session, page):
        ...
        remaining = sum(
            1 for cookie in await session.cookies() if _is_yandex_host(str(cookie["domain"]))
        )
        ...
        return LogoutReport(deleted=len(doomed), remaining=remaining)
